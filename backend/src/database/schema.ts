...
export async function initializeSchema(database: Database): Promise<void> {
  await database.query(`
    ...
    create index if not exists idx_chatbot_intents_site_active
      on chatbot_intents(site_id, is_active, priority desc);
    create index if not exists idx_knowledge_items_site_status
      on knowledge_items(site_id, status, priority desc);
    ...
    create index if not exists idx_chatbot_goals_site
    ...
    create index if not exists idx_chatbot_studios_org_stage
      on chatbot_studios(organization_id, current_stage);
    create index if not exists idx_chatbot_studio_versions_site_created
      on chatbot_studio_versions(site_id, created_at desc);
    create index if not exists idx_chatbot_import_proposals_site_status
      on chatbot_import_proposals(site_id, status, created_at desc);
    create index if not exists idx_chatbot_simulations_site_created
      on chatbot_simulations(site_id, created_at desc);
  `);
}
